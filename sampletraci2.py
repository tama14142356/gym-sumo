    ...
    while True:
        ...
        try:
            route = traci.simulation.findRoute(
                g.getEdgeID(fromedge), g.getEdgeID(toedge)
            )
...
def turn(v, direction):
    ...
    if nextEdgeID is None:
        ...
        pos = min(traci.vehicle.getLength(v), traci.lane.getLength(nextLaneID))
        ...
        newroute = traci.simulation.findRoute(nextEdgeID, routeList[len(routeList) - 1])
...
def takeAction():
    ...
    for v in v_list:
        ...
        try:
            ...
            #     v, current_edge, laneindex, v_x, v_y, angle=angle, keepRoute=4
            # )
            ...
            #     current_edge = traci.vehicle.getRoadID(v)
            #     print(
            #         current_edge, routeIndex, routes, routes[routeIndex], "routeindex"
            #     )
            # if step == 3:
            #     turn(v, "l")
            # print(
            ...
            if v == "veh0":
                ...
                traci.vehicle.moveToXY(v, "", laneindex, v_next_x, v_next_y, angle, 2)
                showInfo(v)
            # if v == v_list[0]:
            #     print(
            #         v,
...
def main(sumocfg):
    ...
    traci.vehicle.subscribe("veh0", (tc.VAR_ROAD_ID, tc.VAR_LANEPOSITION, tc.VAR_ANGLE))
    ...
    while True:  # 1day
        ...
        if isJunction(vehID):
            ...
            if route_index + 2 < len(route):
                ...
                if distance > 0.0:
                    direction = "l"
                    next_next_edgeID = g.getNextInfoTo(next_edgeID, direction)
                    if (
                        next_next_edgeID is not None
                        and next_next_edgeID != route[route_index + 2]
                    ):
                        target = route[len(route) - 1]
                        newroute = traci.simulation.findRoute(next_next_edgeID, target)
                        if len(newroute.edges) > 0:
                            route_route = [next_edgeID]
                            route_route[
                                len(route_route) : len(newroute.edges)
                            ] = newroute.edges
